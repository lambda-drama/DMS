"use strict";
(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
	[878],
	{
		63509: (e, t, n) => {
			n.d(t, { X: () => o });
			var r = n(12115),
				i = n(66294);
			function o(e) {
				let [t, n] = r.useState(void 0);
				return (
					(0, i.N)(() => {
						if (e) {
							n({ width: e.offsetWidth, height: e.offsetHeight });
							let t = new ResizeObserver((t) => {
								let r, i;
								if (!Array.isArray(t) || !t.length) return;
								let o = t[0];
								if ("borderBoxSize" in o) {
									let e = o.borderBoxSize,
										t = Array.isArray(e) ? e[0] : e;
									(r = t.inlineSize), (i = t.blockSize);
								} else (r = e.offsetWidth), (i = e.offsetHeight);
								n({ width: r, height: i });
							});
							return t.observe(e, { box: "border-box" }), () => t.unobserve(e);
						}
						n(void 0);
					}, [e]),
					t
				);
			}
		},
		80878: (e, t, n) => {
			let r;
			n.d(t, { Mz: () => eU, i3: () => eK, UC: () => eJ, bL: () => eq, Bk: () => eH });
			var i = n(12115);
			let o = ["top", "right", "bottom", "left"],
				l = Math.min,
				a = Math.max,
				f = Math.round,
				s = Math.floor,
				c = (e) => ({ x: e, y: e }),
				u = { left: "right", right: "left", bottom: "top", top: "bottom" };
			function d(e, t) {
				return "function" == typeof e ? e(t) : e;
			}
			function p(e) {
				return e.split("-")[0];
			}
			function h(e) {
				return e.split("-")[1];
			}
			function m(e) {
				return "x" === e ? "y" : "x";
			}
			function g(e) {
				return "y" === e ? "height" : "width";
			}
			function y(e) {
				let t = e[0];
				return "t" === t || "b" === t ? "y" : "x";
			}
			function w(e) {
				return e.includes("start") ? e.replace("start", "end") : e.replace("end", "start");
			}
			let x = ["left", "right"],
				v = ["right", "left"],
				b = ["top", "bottom"],
				R = ["bottom", "top"];
			function A(e) {
				let t = p(e);
				return u[t] + e.slice(t.length);
			}
			function O(e) {
				return "number" != typeof e
					? { top: 0, right: 0, bottom: 0, left: 0, ...e }
					: { top: e, right: e, bottom: e, left: e };
			}
			function S(e) {
				let { x: t, y: n, width: r, height: i } = e;
				return {
					width: r,
					height: i,
					top: n,
					left: t,
					right: t + r,
					bottom: n + i,
					x: t,
					y: n,
				};
			}
			function L(e, t, n) {
				let r,
					{ reference: i, floating: o } = e,
					l = y(t),
					a = m(y(t)),
					f = g(a),
					s = p(t),
					c = "y" === l,
					u = i.x + i.width / 2 - o.width / 2,
					d = i.y + i.height / 2 - o.height / 2,
					w = i[f] / 2 - o[f] / 2;
				switch (s) {
					case "top":
						r = { x: u, y: i.y - o.height };
						break;
					case "bottom":
						r = { x: u, y: i.y + i.height };
						break;
					case "right":
						r = { x: i.x + i.width, y: d };
						break;
					case "left":
						r = { x: i.x - o.width, y: d };
						break;
					default:
						r = { x: i.x, y: i.y };
				}
				switch (h(t)) {
					case "start":
						r[a] -= w * (n && c ? -1 : 1);
						break;
					case "end":
						r[a] += w * (n && c ? -1 : 1);
				}
				return r;
			}
			async function C(e, t) {
				var n;
				void 0 === t && (t = {});
				let { x: r, y: i, platform: o, rects: l, elements: a, strategy: f } = e,
					{
						boundary: s = "clippingAncestors",
						rootBoundary: c = "viewport",
						elementContext: u = "floating",
						altBoundary: p = !1,
						padding: h = 0,
					} = d(t, e),
					m = O(h),
					g = a[p ? ("floating" === u ? "reference" : "floating") : u],
					y = S(
						await o.getClippingRect({
							element:
								null ==
									(n = await (null == o.isElement ? void 0 : o.isElement(g))) ||
								n
									? g
									: g.contextElement ||
									  (await (null == o.getDocumentElement
											? void 0
											: o.getDocumentElement(a.floating))),
							boundary: s,
							rootBoundary: c,
							strategy: f,
						})
					),
					w =
						"floating" === u
							? { x: r, y: i, width: l.floating.width, height: l.floating.height }
							: l.reference,
					x = await (null == o.getOffsetParent ? void 0 : o.getOffsetParent(a.floating)),
					v = ((await (null == o.isElement ? void 0 : o.isElement(x))) &&
						(await (null == o.getScale ? void 0 : o.getScale(x)))) || { x: 1, y: 1 },
					b = S(
						o.convertOffsetParentRelativeRectToViewportRelativeRect
							? await o.convertOffsetParentRelativeRectToViewportRelativeRect({
									elements: a,
									rect: w,
									offsetParent: x,
									strategy: f,
							  })
							: w
					);
				return {
					top: (y.top - b.top + m.top) / v.y,
					bottom: (b.bottom - y.bottom + m.bottom) / v.y,
					left: (y.left - b.left + m.left) / v.x,
					right: (b.right - y.right + m.right) / v.x,
				};
			}
			let E = async (e, t, n) => {
				let {
						placement: r = "bottom",
						strategy: i = "absolute",
						middleware: o = [],
						platform: l,
					} = n,
					a = l.detectOverflow ? l : { ...l, detectOverflow: C },
					f = await (null == l.isRTL ? void 0 : l.isRTL(t)),
					s = await l.getElementRects({ reference: e, floating: t, strategy: i }),
					{ x: c, y: u } = L(s, r, f),
					d = r,
					p = 0,
					h = {};
				for (let n = 0; n < o.length; n++) {
					let m = o[n];
					if (!m) continue;
					let { name: g, fn: y } = m,
						{
							x: w,
							y: x,
							data: v,
							reset: b,
						} = await y({
							x: c,
							y: u,
							initialPlacement: r,
							placement: d,
							strategy: i,
							middlewareData: h,
							rects: s,
							platform: a,
							elements: { reference: e, floating: t },
						});
					(c = null != w ? w : c),
						(u = null != x ? x : u),
						(h[g] = { ...h[g], ...v }),
						b &&
							p < 50 &&
							(p++,
							"object" == typeof b &&
								(b.placement && (d = b.placement),
								b.rects &&
									(s =
										!0 === b.rects
											? await l.getElementRects({
													reference: e,
													floating: t,
													strategy: i,
											  })
											: b.rects),
								({ x: c, y: u } = L(s, d, f))),
							(n = -1));
				}
				return { x: c, y: u, placement: d, strategy: i, middlewareData: h };
			};
			function T(e, t) {
				return {
					top: e.top - t.height,
					right: e.right - t.width,
					bottom: e.bottom - t.height,
					left: e.left - t.width,
				};
			}
			function P(e) {
				return o.some((t) => e[t] >= 0);
			}
			let k = new Set(["left", "top"]);
			async function H(e, t) {
				let { placement: n, platform: r, elements: i } = e,
					o = await (null == r.isRTL ? void 0 : r.isRTL(i.floating)),
					l = p(n),
					a = h(n),
					f = "y" === y(n),
					s = k.has(l) ? -1 : 1,
					c = o && f ? -1 : 1,
					u = d(t, e),
					{
						mainAxis: m,
						crossAxis: g,
						alignmentAxis: w,
					} = "number" == typeof u
						? { mainAxis: u, crossAxis: 0, alignmentAxis: null }
						: {
								mainAxis: u.mainAxis || 0,
								crossAxis: u.crossAxis || 0,
								alignmentAxis: u.alignmentAxis,
						  };
				return (
					a && "number" == typeof w && (g = "end" === a ? -1 * w : w),
					f ? { x: g * c, y: m * s } : { x: m * s, y: g * c }
				);
			}
			function D() {
				return "u" > typeof window;
			}
			function F(e) {
				return $(e) ? (e.nodeName || "").toLowerCase() : "#document";
			}
			function M(e) {
				var t;
				return (
					(null == e || null == (t = e.ownerDocument) ? void 0 : t.defaultView) || window
				);
			}
			function W(e) {
				var t;
				return null == (t = ($(e) ? e.ownerDocument : e.document) || window.document)
					? void 0
					: t.documentElement;
			}
			function $(e) {
				return !!D() && (e instanceof Node || e instanceof M(e).Node);
			}
			function N(e) {
				return !!D() && (e instanceof Element || e instanceof M(e).Element);
			}
			function j(e) {
				return !!D() && (e instanceof HTMLElement || e instanceof M(e).HTMLElement);
			}
			function z(e) {
				return (
					!(!D() || "u" < typeof ShadowRoot) &&
					(e instanceof ShadowRoot || e instanceof M(e).ShadowRoot)
				);
			}
			function B(e) {
				let { overflow: t, overflowX: n, overflowY: r, display: i } = U(e);
				return (
					/auto|scroll|overlay|hidden|clip/.test(t + r + n) &&
					"inline" !== i &&
					"contents" !== i
				);
			}
			function V(e) {
				try {
					if (e.matches(":popover-open")) return !0;
				} catch (e) {}
				try {
					return e.matches(":modal");
				} catch (e) {
					return !1;
				}
			}
			let _ = /transform|translate|scale|rotate|perspective|filter/,
				X = /paint|layout|strict|content/,
				I = (e) => !!e && "none" !== e;
			function Y(e) {
				let t = N(e) ? U(e) : e;
				return (
					I(t.transform) ||
					I(t.translate) ||
					I(t.scale) ||
					I(t.rotate) ||
					I(t.perspective) ||
					(!G() && (I(t.backdropFilter) || I(t.filter))) ||
					_.test(t.willChange || "") ||
					X.test(t.contain || "")
				);
			}
			function G() {
				return (
					null == r &&
						(r =
							"u" > typeof CSS &&
							CSS.supports &&
							CSS.supports("-webkit-backdrop-filter", "none")),
					r
				);
			}
			function q(e) {
				return /^(html|body|#document)$/.test(F(e));
			}
			function U(e) {
				return M(e).getComputedStyle(e);
			}
			function J(e) {
				return N(e)
					? { scrollLeft: e.scrollLeft, scrollTop: e.scrollTop }
					: { scrollLeft: e.scrollX, scrollTop: e.scrollY };
			}
			function K(e) {
				if ("html" === F(e)) return e;
				let t = e.assignedSlot || e.parentNode || (z(e) && e.host) || W(e);
				return z(t) ? t.host : t;
			}
			function Q(e, t, n) {
				var r;
				void 0 === t && (t = []), void 0 === n && (n = !0);
				let i = (function e(t) {
						let n = K(t);
						return q(n)
							? t.ownerDocument
								? t.ownerDocument.body
								: t.body
							: j(n) && B(n)
							? n
							: e(n);
					})(e),
					o = i === (null == (r = e.ownerDocument) ? void 0 : r.body),
					l = M(i);
				if (!o) return t.concat(i, Q(i, [], n));
				{
					let e = Z(l);
					return t.concat(l, l.visualViewport || [], B(i) ? i : [], e && n ? Q(e) : []);
				}
			}
			function Z(e) {
				return e.parent && Object.getPrototypeOf(e.parent) ? e.frameElement : null;
			}
			function ee(e) {
				let t = U(e),
					n = parseFloat(t.width) || 0,
					r = parseFloat(t.height) || 0,
					i = j(e),
					o = i ? e.offsetWidth : n,
					l = i ? e.offsetHeight : r,
					a = f(n) !== o || f(r) !== l;
				return a && ((n = o), (r = l)), { width: n, height: r, $: a };
			}
			function et(e) {
				return N(e) ? e : e.contextElement;
			}
			function en(e) {
				let t = et(e);
				if (!j(t)) return c(1);
				let n = t.getBoundingClientRect(),
					{ width: r, height: i, $: o } = ee(t),
					l = (o ? f(n.width) : n.width) / r,
					a = (o ? f(n.height) : n.height) / i;
				return (
					(l && Number.isFinite(l)) || (l = 1),
					(a && Number.isFinite(a)) || (a = 1),
					{ x: l, y: a }
				);
			}
			let er = c(0);
			function ei(e) {
				let t = M(e);
				return G() && t.visualViewport
					? { x: t.visualViewport.offsetLeft, y: t.visualViewport.offsetTop }
					: er;
			}
			function eo(e, t, n, r) {
				var i;
				void 0 === t && (t = !1), void 0 === n && (n = !1);
				let o = e.getBoundingClientRect(),
					l = et(e),
					a = c(1);
				t && (r ? N(r) && (a = en(r)) : (a = en(e)));
				let f = (void 0 === (i = n) && (i = !1), r && (!i || r === M(l)) && i)
						? ei(l)
						: c(0),
					s = (o.left + f.x) / a.x,
					u = (o.top + f.y) / a.y,
					d = o.width / a.x,
					p = o.height / a.y;
				if (l) {
					let e = M(l),
						t = r && N(r) ? M(r) : r,
						n = e,
						i = Z(n);
					for (; i && r && t !== n; ) {
						let e = en(i),
							t = i.getBoundingClientRect(),
							r = U(i),
							o = t.left + (i.clientLeft + parseFloat(r.paddingLeft)) * e.x,
							l = t.top + (i.clientTop + parseFloat(r.paddingTop)) * e.y;
						(s *= e.x),
							(u *= e.y),
							(d *= e.x),
							(p *= e.y),
							(s += o),
							(u += l),
							(i = Z((n = M(i))));
					}
				}
				return S({ width: d, height: p, x: s, y: u });
			}
			function el(e, t) {
				let n = J(e).scrollLeft;
				return t ? t.left + n : eo(W(e)).left + n;
			}
			function ea(e, t) {
				let n = e.getBoundingClientRect();
				return { x: n.left + t.scrollLeft - el(e, n), y: n.top + t.scrollTop };
			}
			function ef(e, t, n) {
				var r;
				let i;
				if ("viewport" === t)
					i = (function (e, t) {
						let n = M(e),
							r = W(e),
							i = n.visualViewport,
							o = r.clientWidth,
							l = r.clientHeight,
							a = 0,
							f = 0;
						if (i) {
							(o = i.width), (l = i.height);
							let e = G();
							(!e || (e && "fixed" === t)) &&
								((a = i.offsetLeft), (f = i.offsetTop));
						}
						let s = el(r);
						if (s <= 0) {
							let e = r.ownerDocument,
								t = e.body,
								n = getComputedStyle(t),
								i =
									("CSS1Compat" === e.compatMode &&
										parseFloat(n.marginLeft) + parseFloat(n.marginRight)) ||
									0,
								l = Math.abs(r.clientWidth - t.clientWidth - i);
							l <= 25 && (o -= l);
						} else s <= 25 && (o += s);
						return { width: o, height: l, x: a, y: f };
					})(e, n);
				else if ("document" === t) {
					let t, n, o, l, f, s, c;
					(r = W(e)),
						(t = W(r)),
						(n = J(r)),
						(o = r.ownerDocument.body),
						(l = a(t.scrollWidth, t.clientWidth, o.scrollWidth, o.clientWidth)),
						(f = a(t.scrollHeight, t.clientHeight, o.scrollHeight, o.clientHeight)),
						(s = -n.scrollLeft + el(r)),
						(c = -n.scrollTop),
						"rtl" === U(o).direction && (s += a(t.clientWidth, o.clientWidth) - l),
						(i = { width: l, height: f, x: s, y: c });
				} else if (N(t)) {
					let e, r, o, l, a, f;
					(r = (e = eo(t, !0, "fixed" === n)).top + t.clientTop),
						(o = e.left + t.clientLeft),
						(l = j(t) ? en(t) : c(1)),
						(a = t.clientWidth * l.x),
						(f = t.clientHeight * l.y),
						(i = { width: a, height: f, x: o * l.x, y: r * l.y });
				} else {
					let n = ei(e);
					i = { x: t.x - n.x, y: t.y - n.y, width: t.width, height: t.height };
				}
				return S(i);
			}
			function es(e) {
				return "static" === U(e).position;
			}
			function ec(e, t) {
				if (!j(e) || "fixed" === U(e).position) return null;
				if (t) return t(e);
				let n = e.offsetParent;
				return W(e) === n && (n = n.ownerDocument.body), n;
			}
			function eu(e, t) {
				var n;
				let r = M(e);
				if (V(e)) return r;
				if (!j(e)) {
					let t = K(e);
					for (; t && !q(t); ) {
						if (N(t) && !es(t)) return t;
						t = K(t);
					}
					return r;
				}
				let i = ec(e, t);
				for (; i && ((n = i), /^(table|td|th)$/.test(F(n))) && es(i); ) i = ec(i, t);
				return i && q(i) && es(i) && !Y(i)
					? r
					: i ||
							(function (e) {
								let t = K(e);
								for (; j(t) && !q(t); ) {
									if (Y(t)) return t;
									if (V(t)) break;
									t = K(t);
								}
								return null;
							})(e) ||
							r;
			}
			let ed = async function (e) {
					let t = this.getOffsetParent || eu,
						n = this.getDimensions,
						r = await n(e.floating);
					return {
						reference: (function (e, t, n) {
							let r = j(t),
								i = W(t),
								o = "fixed" === n,
								l = eo(e, !0, o, t),
								a = { scrollLeft: 0, scrollTop: 0 },
								f = c(0);
							if (r || (!r && !o))
								if ((("body" !== F(t) || B(i)) && (a = J(t)), r)) {
									let e = eo(t, !0, o, t);
									(f.x = e.x + t.clientLeft), (f.y = e.y + t.clientTop);
								} else i && (f.x = el(i));
							o && !r && i && (f.x = el(i));
							let s = !i || r || o ? c(0) : ea(i, a);
							return {
								x: l.left + a.scrollLeft - f.x - s.x,
								y: l.top + a.scrollTop - f.y - s.y,
								width: l.width,
								height: l.height,
							};
						})(e.reference, await t(e.floating), e.strategy),
						floating: { x: 0, y: 0, width: r.width, height: r.height },
					};
				},
				ep = {
					convertOffsetParentRelativeRectToViewportRelativeRect: function (e) {
						let { elements: t, rect: n, offsetParent: r, strategy: i } = e,
							o = "fixed" === i,
							l = W(r),
							a = !!t && V(t.floating);
						if (r === l || (a && o)) return n;
						let f = { scrollLeft: 0, scrollTop: 0 },
							s = c(1),
							u = c(0),
							d = j(r);
						if ((d || (!d && !o)) && (("body" !== F(r) || B(l)) && (f = J(r)), d)) {
							let e = eo(r);
							(s = en(r)), (u.x = e.x + r.clientLeft), (u.y = e.y + r.clientTop);
						}
						let p = !l || d || o ? c(0) : ea(l, f);
						return {
							width: n.width * s.x,
							height: n.height * s.y,
							x: n.x * s.x - f.scrollLeft * s.x + u.x + p.x,
							y: n.y * s.y - f.scrollTop * s.y + u.y + p.y,
						};
					},
					getDocumentElement: W,
					getClippingRect: function (e) {
						let { element: t, boundary: n, rootBoundary: r, strategy: i } = e,
							o = [
								...("clippingAncestors" === n
									? V(t)
										? []
										: (function (e, t) {
												let n = t.get(e);
												if (n) return n;
												let r = Q(e, [], !1).filter(
														(e) => N(e) && "body" !== F(e)
													),
													i = null,
													o = "fixed" === U(e).position,
													l = o ? K(e) : e;
												for (; N(l) && !q(l); ) {
													let t = U(l),
														n = Y(l);
													n || "fixed" !== t.position || (i = null),
														(
															o
																? n || i
																: !(
																		(!n &&
																			"static" ===
																				t.position &&
																			i &&
																			("absolute" ===
																				i.position ||
																				"fixed" ===
																					i.position)) ||
																		(B(l) &&
																			!n &&
																			(function e(t, n) {
																				let r = K(t);
																				return (
																					!(
																						r === n ||
																						!N(r) ||
																						q(r)
																					) &&
																					("fixed" ===
																						U(r)
																							.position ||
																						e(r, n))
																				);
																			})(e, l))
																  )
														)
															? (i = t)
															: (r = r.filter((e) => e !== l)),
														(l = K(l));
												}
												return t.set(e, r), r;
										  })(t, this._c)
									: [].concat(n)),
								r,
							],
							f = ef(t, o[0], i),
							s = f.top,
							c = f.right,
							u = f.bottom,
							d = f.left;
						for (let e = 1; e < o.length; e++) {
							let n = ef(t, o[e], i);
							(s = a(n.top, s)),
								(c = l(n.right, c)),
								(u = l(n.bottom, u)),
								(d = a(n.left, d));
						}
						return { width: c - d, height: u - s, x: d, y: s };
					},
					getOffsetParent: eu,
					getElementRects: ed,
					getClientRects: function (e) {
						return Array.from(e.getClientRects());
					},
					getDimensions: function (e) {
						let { width: t, height: n } = ee(e);
						return { width: t, height: n };
					},
					getScale: en,
					isElement: N,
					isRTL: function (e) {
						return "rtl" === U(e).direction;
					},
				};
			function eh(e, t) {
				return e.x === t.x && e.y === t.y && e.width === t.width && e.height === t.height;
			}
			let em = (e) => ({
				name: "arrow",
				options: e,
				async fn(t) {
					let {
							x: n,
							y: r,
							placement: i,
							rects: o,
							platform: f,
							elements: s,
							middlewareData: c,
						} = t,
						{ element: u, padding: p = 0 } = d(e, t) || {};
					if (null == u) return {};
					let w = O(p),
						x = { x: n, y: r },
						v = m(y(i)),
						b = g(v),
						R = await f.getDimensions(u),
						A = "y" === v,
						S = A ? "clientHeight" : "clientWidth",
						L = o.reference[b] + o.reference[v] - x[v] - o.floating[b],
						C = x[v] - o.reference[v],
						E = await (null == f.getOffsetParent ? void 0 : f.getOffsetParent(u)),
						T = E ? E[S] : 0;
					(T && (await (null == f.isElement ? void 0 : f.isElement(E)))) ||
						(T = s.floating[S] || o.floating[b]);
					let P = T / 2 - R[b] / 2 - 1,
						k = l(w[A ? "top" : "left"], P),
						H = l(w[A ? "bottom" : "right"], P),
						D = T - R[b] - H,
						F = T / 2 - R[b] / 2 + (L / 2 - C / 2),
						M = a(k, l(F, D)),
						W =
							!c.arrow &&
							null != h(i) &&
							F !== M &&
							o.reference[b] / 2 - (F < k ? k : H) - R[b] / 2 < 0,
						$ = W ? (F < k ? F - k : F - D) : 0;
					return {
						[v]: x[v] + $,
						data: {
							[v]: M,
							centerOffset: F - M - $,
							...(W && { alignmentOffset: $ }),
						},
						reset: W,
					};
				},
			});
			var eg = n(47650),
				ey = "u" > typeof document ? i.useLayoutEffect : function () {};
			function ew(e, t) {
				let n, r, i;
				if (e === t) return !0;
				if (typeof e != typeof t) return !1;
				if ("function" == typeof e && e.toString() === t.toString()) return !0;
				if (e && t && "object" == typeof e) {
					if (Array.isArray(e)) {
						if ((n = e.length) !== t.length) return !1;
						for (r = n; 0 != r--; ) if (!ew(e[r], t[r])) return !1;
						return !0;
					}
					if ((n = (i = Object.keys(e)).length) !== Object.keys(t).length) return !1;
					for (r = n; 0 != r--; ) if (!{}.hasOwnProperty.call(t, i[r])) return !1;
					for (r = n; 0 != r--; ) {
						let n = i[r];
						if (("_owner" !== n || !e.$$typeof) && !ew(e[n], t[n])) return !1;
					}
					return !0;
				}
				return e != e && t != t;
			}
			function ex(e) {
				return "u" < typeof window
					? 1
					: (e.ownerDocument.defaultView || window).devicePixelRatio || 1;
			}
			function ev(e, t) {
				let n = ex(e);
				return Math.round(t * n) / n;
			}
			function eb(e) {
				let t = i.useRef(e);
				return (
					ey(() => {
						t.current = e;
					}),
					t
				);
			}
			var eR = n(99354),
				eA = n(95155),
				eO = i.forwardRef((e, t) => {
					let { children: n, width: r = 10, height: i = 5, ...o } = e;
					return (0, eA.jsx)(eR.sG.svg, {
						...o,
						ref: t,
						width: r,
						height: i,
						viewBox: "0 0 30 10",
						preserveAspectRatio: "none",
						children: e.asChild
							? n
							: (0, eA.jsx)("polygon", { points: "0,0 30,0 15,10" }),
					});
				});
			eO.displayName = "Arrow";
			var eS = n(47527),
				eL = n(68599),
				eC = n(17347),
				eE = n(66294),
				eT = n(63509),
				eP = "Popper",
				[ek, eH] = (0, eL.A)(eP),
				[eD, eF] = ek(eP),
				eM = (e) => {
					let { __scopePopper: t, children: n } = e,
						[r, o] = i.useState(null);
					return (0, eA.jsx)(eD, {
						scope: t,
						anchor: r,
						onAnchorChange: o,
						children: n,
					});
				};
			eM.displayName = eP;
			var eW = "PopperAnchor",
				e$ = i.forwardRef((e, t) => {
					let { __scopePopper: n, virtualRef: r, ...o } = e,
						l = eF(eW, n),
						a = i.useRef(null),
						f = (0, eS.s)(t, a),
						s = i.useRef(null);
					return (
						i.useEffect(() => {
							let e = s.current;
							(s.current = r?.current || a.current),
								e !== s.current && l.onAnchorChange(s.current);
						}),
						r ? null : (0, eA.jsx)(eR.sG.div, { ...o, ref: f })
					);
				});
			e$.displayName = eW;
			var eN = "PopperContent",
				[ej, ez] = ek(eN),
				eB = i.forwardRef((e, t) => {
					var n, r, o, f, c, u, O, S, L, C, D, F, M;
					let $,
						N,
						j,
						z,
						B,
						V,
						{
							__scopePopper: _,
							side: X = "bottom",
							sideOffset: I = 0,
							align: Y = "center",
							alignOffset: G = 0,
							arrowPadding: q = 0,
							avoidCollisions: U = !0,
							collisionBoundary: J = [],
							collisionPadding: K = 0,
							sticky: Z = "partial",
							hideWhenDetached: ee = !1,
							updatePositionStrategy: en = "optimized",
							onPlaced: er,
							...ei
						} = e,
						el = eF(eN, _),
						[ea, ef] = i.useState(null),
						es = (0, eS.s)(t, (e) => ef(e)),
						[ec, eu] = i.useState(null),
						ed = (0, eT.X)(ec),
						eO = ed?.width ?? 0,
						eL = ed?.height ?? 0,
						eP =
							"number" == typeof K
								? K
								: { top: 0, right: 0, bottom: 0, left: 0, ...K },
						ek = Array.isArray(J) ? J : [J],
						eH = ek.length > 0,
						eD = { padding: eP, boundary: ek.filter(eI), altBoundary: eH },
						{
							refs: eM,
							floatingStyles: eW,
							placement: e$,
							isPositioned: ez,
							middlewareData: eB,
						} = (function (e) {
							void 0 === e && (e = {});
							let {
									placement: t = "bottom",
									strategy: n = "absolute",
									middleware: r = [],
									platform: o,
									elements: { reference: l, floating: a } = {},
									transform: f = !0,
									whileElementsMounted: s,
									open: c,
								} = e,
								[u, d] = i.useState({
									x: 0,
									y: 0,
									strategy: n,
									placement: t,
									middlewareData: {},
									isPositioned: !1,
								}),
								[p, h] = i.useState(r);
							ew(p, r) || h(r);
							let [m, g] = i.useState(null),
								[y, w] = i.useState(null),
								x = i.useCallback((e) => {
									e !== A.current && ((A.current = e), g(e));
								}, []),
								v = i.useCallback((e) => {
									e !== O.current && ((O.current = e), w(e));
								}, []),
								b = l || m,
								R = a || y,
								A = i.useRef(null),
								O = i.useRef(null),
								S = i.useRef(u),
								L = null != s,
								C = eb(s),
								T = eb(o),
								P = eb(c),
								k = i.useCallback(() => {
									var e, r;
									let i, o, l;
									if (!A.current || !O.current) return;
									let a = { placement: t, strategy: n, middleware: p };
									T.current && (a.platform = T.current),
										((e = A.current),
										(r = O.current),
										(i = new Map()),
										(l = { ...(o = { platform: ep, ...a }).platform, _c: i }),
										E(e, r, { ...o, platform: l })).then((e) => {
											let t = { ...e, isPositioned: !1 !== P.current };
											H.current &&
												!ew(S.current, t) &&
												((S.current = t),
												eg.flushSync(() => {
													d(t);
												}));
										});
								}, [p, t, n, T, P]);
							ey(() => {
								!1 === c &&
									S.current.isPositioned &&
									((S.current.isPositioned = !1),
									d((e) => ({ ...e, isPositioned: !1 })));
							}, [c]);
							let H = i.useRef(!1);
							ey(
								() => (
									(H.current = !0),
									() => {
										H.current = !1;
									}
								),
								[]
							),
								ey(() => {
									if ((b && (A.current = b), R && (O.current = R), b && R)) {
										if (C.current) return C.current(b, R, k);
										k();
									}
								}, [b, R, k, C, L]);
							let D = i.useMemo(
									() => ({
										reference: A,
										floating: O,
										setReference: x,
										setFloating: v,
									}),
									[x, v]
								),
								F = i.useMemo(() => ({ reference: b, floating: R }), [b, R]),
								M = i.useMemo(() => {
									let e = { position: n, left: 0, top: 0 };
									if (!F.floating) return e;
									let t = ev(F.floating, u.x),
										r = ev(F.floating, u.y);
									return f
										? {
												...e,
												transform: "translate(" + t + "px, " + r + "px)",
												...(ex(F.floating) >= 1.5 && {
													willChange: "transform",
												}),
										  }
										: { position: n, left: t, top: r };
								}, [n, f, F.floating, u.x, u.y]);
							return i.useMemo(
								() => ({
									...u,
									update: k,
									refs: D,
									elements: F,
									floatingStyles: M,
								}),
								[u, k, D, F, M]
							);
						})({
							strategy: "fixed",
							placement: X + ("center" !== Y ? "-" + Y : ""),
							whileElementsMounted: (...e) =>
								(function (e, t, n, r) {
									let i;
									void 0 === r && (r = {});
									let {
											ancestorScroll: o = !0,
											ancestorResize: f = !0,
											elementResize: c = "function" == typeof ResizeObserver,
											layoutShift: u = "function" ==
												typeof IntersectionObserver,
											animationFrame: d = !1,
										} = r,
										p = et(e),
										h = o || f ? [...(p ? Q(p) : []), ...(t ? Q(t) : [])] : [];
									h.forEach((e) => {
										o && e.addEventListener("scroll", n, { passive: !0 }),
											f && e.addEventListener("resize", n);
									});
									let m =
											p && u
												? (function (e, t) {
														let n,
															r = null,
															i = W(e);
														function o() {
															var e;
															clearTimeout(n),
																null == (e = r) || e.disconnect(),
																(r = null);
														}
														return (
															!(function f(c, u) {
																void 0 === c && (c = !1),
																	void 0 === u && (u = 1),
																	o();
																let d = e.getBoundingClientRect(),
																	{
																		left: p,
																		top: h,
																		width: m,
																		height: g,
																	} = d;
																if ((c || t(), !m || !g)) return;
																let y = {
																		rootMargin:
																			-s(h) +
																			"px " +
																			-s(
																				i.clientWidth -
																					(p + m)
																			) +
																			"px " +
																			-s(
																				i.clientHeight -
																					(h + g)
																			) +
																			"px " +
																			-s(p) +
																			"px",
																		threshold:
																			a(0, l(1, u)) || 1,
																	},
																	w = !0;
																function x(t) {
																	let r = t[0].intersectionRatio;
																	if (r !== u) {
																		if (!w) return f();
																		r
																			? f(!1, r)
																			: (n = setTimeout(
																					() => {
																						f(
																							!1,
																							1e-7
																						);
																					},
																					1e3
																			  ));
																	}
																	1 !== r ||
																		eh(
																			d,
																			e.getBoundingClientRect()
																		) ||
																		f(),
																		(w = !1);
																}
																try {
																	r = new IntersectionObserver(
																		x,
																		{
																			...y,
																			root: i.ownerDocument,
																		}
																	);
																} catch (e) {
																	r = new IntersectionObserver(
																		x,
																		y
																	);
																}
																r.observe(e);
															})(!0),
															o
														);
												  })(p, n)
												: null,
										g = -1,
										y = null;
									c &&
										((y = new ResizeObserver((e) => {
											let [r] = e;
											r &&
												r.target === p &&
												y &&
												t &&
												(y.unobserve(t),
												cancelAnimationFrame(g),
												(g = requestAnimationFrame(() => {
													var e;
													null == (e = y) || e.observe(t);
												}))),
												n();
										})),
										p && !d && y.observe(p),
										t && y.observe(t));
									let w = d ? eo(e) : null;
									return (
										d &&
											(function t() {
												let r = eo(e);
												w && !eh(w, r) && n(),
													(w = r),
													(i = requestAnimationFrame(t));
											})(),
										n(),
										() => {
											var e;
											h.forEach((e) => {
												o && e.removeEventListener("scroll", n),
													f && e.removeEventListener("resize", n);
											}),
												null == m || m(),
												null == (e = y) || e.disconnect(),
												(y = null),
												d && cancelAnimationFrame(i);
										}
									);
								})(...e, { animationFrame: "always" === en }),
							elements: { reference: el.anchor },
							middleware: [
								{
									name: ($ = {
										name: "offset",
										options: (r = n = { mainAxis: I + eL, alignmentAxis: G }),
										async fn(e) {
											var t, n;
											let {
													x: i,
													y: o,
													placement: l,
													middlewareData: a,
												} = e,
												f = await H(e, r);
											return l ===
												(null == (t = a.offset) ? void 0 : t.placement) &&
												null != (n = a.arrow) &&
												n.alignmentOffset
												? {}
												: {
														x: i + f.x,
														y: o + f.y,
														data: { ...f, placement: l },
												  };
										},
									}).name,
									fn: $.fn,
									options: [n, void 0],
								},
								U && {
									name: (N = {
										name: "shift",
										options:
											(c = f =
												{
													mainAxis: !0,
													crossAxis: !1,
													limiter:
														"partial" === Z
															? {
																	fn: ((o = void 0),
																	{
																		options: (o = {}),
																		fn(e) {
																			let {
																					x: t,
																					y: n,
																					placement: r,
																					rects: i,
																					middlewareData:
																						l,
																				} = e,
																				{
																					offset: a = 0,
																					mainAxis:
																						f = !0,
																					crossAxis:
																						s = !0,
																				} = d(o, e),
																				c = { x: t, y: n },
																				u = y(r),
																				h = m(u),
																				g = c[h],
																				w = c[u],
																				x = d(a, e),
																				v =
																					"number" ==
																					typeof x
																						? {
																								mainAxis:
																									x,
																								crossAxis: 0,
																						  }
																						: {
																								mainAxis: 0,
																								crossAxis: 0,
																								...x,
																						  };
																			if (f) {
																				let e =
																						"y" === h
																							? "height"
																							: "width",
																					t =
																						i
																							.reference[
																							h
																						] -
																						i.floating[
																							e
																						] +
																						v.mainAxis,
																					n =
																						i
																							.reference[
																							h
																						] +
																						i
																							.reference[
																							e
																						] -
																						v.mainAxis;
																				g < t
																					? (g = t)
																					: g > n &&
																					  (g = n);
																			}
																			if (s) {
																				var b, R;
																				let e =
																						"y" === h
																							? "width"
																							: "height",
																					t = k.has(
																						p(r)
																					),
																					n =
																						i
																							.reference[
																							u
																						] -
																						i.floating[
																							e
																						] +
																						((t &&
																							(null ==
																							(b =
																								l.offset)
																								? void 0
																								: b[
																										u
																								  ])) ||
																							0) +
																						(t
																							? 0
																							: v.crossAxis),
																					o =
																						i
																							.reference[
																							u
																						] +
																						i
																							.reference[
																							e
																						] +
																						(t
																							? 0
																							: (null ==
																							  (R =
																									l.offset)
																									? void 0
																									: R[
																											u
																									  ]) ||
																							  0) -
																						(t
																							? v.crossAxis
																							: 0);
																				w < n
																					? (w = n)
																					: w > o &&
																					  (w = o);
																			}
																			return {
																				[h]: g,
																				[u]: w,
																			};
																		},
																	}).fn,
																	options: [void 0, void 0],
															  }
															: void 0,
													...eD,
												}),
										async fn(e) {
											let { x: t, y: n, placement: r, platform: i } = e,
												{
													mainAxis: o = !0,
													crossAxis: f = !1,
													limiter: s = {
														fn: (e) => {
															let { x: t, y: n } = e;
															return { x: t, y: n };
														},
													},
													...u
												} = d(c, e),
												h = { x: t, y: n },
												g = await i.detectOverflow(e, u),
												w = y(p(r)),
												x = m(w),
												v = h[x],
												b = h[w];
											if (o) {
												let e = "y" === x ? "top" : "left",
													t = "y" === x ? "bottom" : "right",
													n = v + g[e],
													r = v - g[t];
												v = a(n, l(v, r));
											}
											if (f) {
												let e = "y" === w ? "top" : "left",
													t = "y" === w ? "bottom" : "right",
													n = b + g[e],
													r = b - g[t];
												b = a(n, l(b, r));
											}
											let R = s.fn({ ...e, [x]: v, [w]: b });
											return {
												...R,
												data: {
													x: R.x - t,
													y: R.y - n,
													enabled: { [x]: o, [w]: f },
												},
											};
										},
									}).name,
									fn: N.fn,
									options: [f, void 0],
								},
								U && {
									name: (j = {
										name: "flip",
										options: (O = u = { ...eD }),
										async fn(e) {
											var t, n, r, i, o, l, a, f;
											let s,
												c,
												u,
												{
													placement: S,
													middlewareData: L,
													rects: C,
													initialPlacement: E,
													platform: T,
													elements: P,
												} = e,
												{
													mainAxis: k = !0,
													crossAxis: H = !0,
													fallbackPlacements: D,
													fallbackStrategy: F = "bestFit",
													fallbackAxisSideDirection: M = "none",
													flipAlignment: W = !0,
													...$
												} = d(O, e);
											if (null != (t = L.arrow) && t.alignmentOffset)
												return {};
											let N = p(S),
												j = y(E),
												z = p(E) === E,
												B = await (null == T.isRTL
													? void 0
													: T.isRTL(P.floating)),
												V =
													D ||
													(z || !W
														? [A(E)]
														: ((s = A(E)), [w(E), s, w(s)])),
												_ = "none" !== M;
											!D &&
												_ &&
												V.push(
													...((c = h(E)),
													(u = (function (e, t, n) {
														switch (e) {
															case "top":
															case "bottom":
																if (n) return t ? v : x;
																return t ? x : v;
															case "left":
															case "right":
																return t ? b : R;
															default:
																return [];
														}
													})(p(E), "start" === M, B)),
													c &&
														((u = u.map((e) => e + "-" + c)),
														W && (u = u.concat(u.map(w)))),
													u)
												);
											let X = [E, ...V],
												I = await T.detectOverflow(e, $),
												Y = [],
												G =
													(null == (n = L.flip)
														? void 0
														: n.overflows) || [];
											if ((k && Y.push(I[N]), H)) {
												let e,
													t,
													n,
													r,
													i =
														((l = S),
														(a = C),
														void 0 === (f = B) && (f = !1),
														(e = h(l)),
														(n = g((t = m(y(l))))),
														(r =
															"x" === t
																? e === (f ? "end" : "start")
																	? "right"
																	: "left"
																: "start" === e
																? "bottom"
																: "top"),
														a.reference[n] > a.floating[n] &&
															(r = A(r)),
														[r, A(r)]);
												Y.push(I[i[0]], I[i[1]]);
											}
											if (
												((G = [...G, { placement: S, overflows: Y }]),
												!Y.every((e) => e <= 0))
											) {
												let e =
														((null == (r = L.flip)
															? void 0
															: r.index) || 0) + 1,
													t = X[e];
												if (
													t &&
													("alignment" !== H ||
														j === y(t) ||
														G.every(
															(e) =>
																y(e.placement) !== j ||
																e.overflows[0] > 0
														))
												)
													return {
														data: { index: e, overflows: G },
														reset: { placement: t },
													};
												let n =
													null ==
													(i = G.filter((e) => e.overflows[0] <= 0).sort(
														(e, t) => e.overflows[1] - t.overflows[1]
													)[0])
														? void 0
														: i.placement;
												if (!n)
													switch (F) {
														case "bestFit": {
															let e =
																null ==
																(o = G.filter((e) => {
																	if (_) {
																		let t = y(e.placement);
																		return (
																			t === j || "y" === t
																		);
																	}
																	return !0;
																})
																	.map((e) => [
																		e.placement,
																		e.overflows
																			.filter((e) => e > 0)
																			.reduce(
																				(e, t) => e + t,
																				0
																			),
																	])
																	.sort(
																		(e, t) => e[1] - t[1]
																	)[0])
																	? void 0
																	: o[0];
															e && (n = e);
															break;
														}
														case "initialPlacement":
															n = E;
													}
												if (S !== n) return { reset: { placement: n } };
											}
											return {};
										},
									}).name,
									fn: j.fn,
									options: [u, void 0],
								},
								{
									name: (z = {
										name: "size",
										options:
											(L = S =
												{
													...eD,
													apply: ({
														elements: e,
														rects: t,
														availableWidth: n,
														availableHeight: r,
													}) => {
														let { width: i, height: o } = t.reference,
															l = e.floating.style;
														l.setProperty(
															"--radix-popper-available-width",
															`${n}px`
														),
															l.setProperty(
																"--radix-popper-available-height",
																`${r}px`
															),
															l.setProperty(
																"--radix-popper-anchor-width",
																`${i}px`
															),
															l.setProperty(
																"--radix-popper-anchor-height",
																`${o}px`
															);
													},
												}),
										async fn(e) {
											var t, n;
											let r,
												i,
												{
													placement: o,
													rects: f,
													platform: s,
													elements: c,
												} = e,
												{ apply: u = () => {}, ...m } = d(L, e),
												g = await s.detectOverflow(e, m),
												w = p(o),
												x = h(o),
												v = "y" === y(o),
												{ width: b, height: R } = f.floating;
											"top" === w || "bottom" === w
												? ((r = w),
												  (i =
														x ===
														((await (null == s.isRTL
															? void 0
															: s.isRTL(c.floating)))
															? "start"
															: "end")
															? "left"
															: "right"))
												: ((i = w), (r = "end" === x ? "top" : "bottom"));
											let A = R - g.top - g.bottom,
												O = b - g.left - g.right,
												S = l(R - g[r], A),
												C = l(b - g[i], O),
												E = !e.middlewareData.shift,
												T = S,
												P = C;
											if (
												(null != (t = e.middlewareData.shift) &&
													t.enabled.x &&
													(P = O),
												null != (n = e.middlewareData.shift) &&
													n.enabled.y &&
													(T = A),
												E && !x)
											) {
												let e = a(g.left, 0),
													t = a(g.right, 0),
													n = a(g.top, 0),
													r = a(g.bottom, 0);
												v
													? (P =
															b -
															2 *
																(0 !== e || 0 !== t
																	? e + t
																	: a(g.left, g.right)))
													: (T =
															R -
															2 *
																(0 !== n || 0 !== r
																	? n + r
																	: a(g.top, g.bottom)));
											}
											await u({
												...e,
												availableWidth: P,
												availableHeight: T,
											});
											let k = await s.getDimensions(c.floating);
											return b !== k.width || R !== k.height
												? { reset: { rects: !0 } }
												: {};
										},
									}).name,
									fn: z.fn,
									options: [S, void 0],
								},
								ec && {
									name: (B = {
										name: "arrow",
										options: (D = C = { element: ec, padding: q }),
										fn(e) {
											let { element: t, padding: n } =
												"function" == typeof D ? D(e) : D;
											return t && {}.hasOwnProperty.call(t, "current")
												? null != t.current
													? em({ element: t.current, padding: n }).fn(e)
													: {}
												: t
												? em({ element: t, padding: n }).fn(e)
												: {};
										},
									}).name,
									fn: B.fn,
									options: [C, void 0],
								},
								eY({ arrowWidth: eO, arrowHeight: eL }),
								ee && {
									name: (V = {
										name: "hide",
										options: (M = F = { strategy: "referenceHidden", ...eD }),
										async fn(e) {
											let { rects: t, platform: n } = e,
												{ strategy: r = "referenceHidden", ...i } = d(
													M,
													e
												);
											switch (r) {
												case "referenceHidden": {
													let r = T(
														await n.detectOverflow(e, {
															...i,
															elementContext: "reference",
														}),
														t.reference
													);
													return {
														data: {
															referenceHiddenOffsets: r,
															referenceHidden: P(r),
														},
													};
												}
												case "escaped": {
													let r = T(
														await n.detectOverflow(e, {
															...i,
															altBoundary: !0,
														}),
														t.floating
													);
													return {
														data: { escapedOffsets: r, escaped: P(r) },
													};
												}
												default:
													return {};
											}
										},
									}).name,
									fn: V.fn,
									options: [F, void 0],
								},
							],
						}),
						[eV, e_] = eG(e$),
						eX = (0, eC.c)(er);
					(0, eE.N)(() => {
						ez && eX?.();
					}, [ez, eX]);
					let eq = eB.arrow?.x,
						eU = eB.arrow?.y,
						eJ = eB.arrow?.centerOffset !== 0,
						[eK, eQ] = i.useState();
					return (
						(0, eE.N)(() => {
							ea && eQ(window.getComputedStyle(ea).zIndex);
						}, [ea]),
						(0, eA.jsx)("div", {
							ref: eM.setFloating,
							"data-radix-popper-content-wrapper": "",
							style: {
								...eW,
								transform: ez ? eW.transform : "translate(0, -200%)",
								minWidth: "max-content",
								zIndex: eK,
								"--radix-popper-transform-origin": [
									eB.transformOrigin?.x,
									eB.transformOrigin?.y,
								].join(" "),
								...(eB.hide?.referenceHidden && {
									visibility: "hidden",
									pointerEvents: "none",
								}),
							},
							dir: e.dir,
							children: (0, eA.jsx)(ej, {
								scope: _,
								placedSide: eV,
								onArrowChange: eu,
								arrowX: eq,
								arrowY: eU,
								shouldHideArrow: eJ,
								children: (0, eA.jsx)(eR.sG.div, {
									"data-side": eV,
									"data-align": e_,
									...ei,
									ref: es,
									style: { ...ei.style, animation: ez ? void 0 : "none" },
								}),
							}),
						})
					);
				});
			eB.displayName = eN;
			var eV = "PopperArrow",
				e_ = { top: "bottom", right: "left", bottom: "top", left: "right" },
				eX = i.forwardRef(function (e, t) {
					let { __scopePopper: n, ...r } = e,
						i = ez(eV, n),
						o = e_[i.placedSide];
					return (0,
					eA.jsx)("span", { ref: i.onArrowChange, style: { position: "absolute", left: i.arrowX, top: i.arrowY, [o]: 0, transformOrigin: { top: "", right: "0 0", bottom: "center 0", left: "100% 0" }[i.placedSide], transform: { top: "translateY(100%)", right: "translateY(50%) rotate(90deg) translateX(-50%)", bottom: "rotate(180deg)", left: "translateY(50%) rotate(-90deg) translateX(50%)" }[i.placedSide], visibility: i.shouldHideArrow ? "hidden" : void 0 }, children: (0, eA.jsx)(eO, { ...r, ref: t, style: { ...r.style, display: "block" } }) });
				});
			function eI(e) {
				return null !== e;
			}
			eX.displayName = eV;
			var eY = (e) => ({
				name: "transformOrigin",
				options: e,
				fn(t) {
					let { placement: n, rects: r, middlewareData: i } = t,
						o = i.arrow?.centerOffset !== 0,
						l = o ? 0 : e.arrowWidth,
						a = o ? 0 : e.arrowHeight,
						[f, s] = eG(n),
						c = { start: "0%", center: "50%", end: "100%" }[s],
						u = (i.arrow?.x ?? 0) + l / 2,
						d = (i.arrow?.y ?? 0) + a / 2,
						p = "",
						h = "";
					return (
						"bottom" === f
							? ((p = o ? c : `${u}px`), (h = `${-a}px`))
							: "top" === f
							? ((p = o ? c : `${u}px`), (h = `${r.floating.height + a}px`))
							: "right" === f
							? ((p = `${-a}px`), (h = o ? c : `${d}px`))
							: "left" === f &&
							  ((p = `${r.floating.width + a}px`), (h = o ? c : `${d}px`)),
						{ data: { x: p, y: h } }
					);
				},
			});
			function eG(e) {
				let [t, n = "center"] = e.split("-");
				return [t, n];
			}
			var eq = eM,
				eU = e$,
				eJ = eB,
				eK = eX;
		},
	},
]);

"use strict";
(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
	[4214],
	{
		12651: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("circle-check", [
				["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
				["path", { d: "m9 12 2 2 4-4", key: "dzmm74" }],
			]);
		},
		13545: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("circle-alert", [
				["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
				["line", { x1: "12", x2: "12", y1: "8", y2: "12", key: "1pkeuh" }],
				["line", { x1: "12", x2: "12.01", y1: "16", y2: "16", key: "4dfq90" }],
			]);
		},
		14636: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("car", [
				[
					"path",
					{
						d: "M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2",
						key: "5owen",
					},
				],
				["circle", { cx: "7", cy: "17", r: "2", key: "u2ysq9" }],
				["path", { d: "M9 17h6", key: "r8uit2" }],
				["circle", { cx: "17", cy: "17", r: "2", key: "axvx0g" }],
			]);
		},
		21283: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("stethoscope", [
				["path", { d: "M11 2v2", key: "1539x4" }],
				["path", { d: "M5 2v2", key: "1yf1q8" }],
				[
					"path",
					{
						d: "M5 3H4a2 2 0 0 0-2 2v4a6 6 0 0 0 12 0V5a2 2 0 0 0-2-2h-1",
						key: "rb5t3r",
					},
				],
				["path", { d: "M8 15a6 6 0 0 0 12 0v-3", key: "x18d4x" }],
				["circle", { cx: "20", cy: "10", r: "2", key: "ts1r5v" }],
			]);
		},
		21362: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("chevron-right", [
				["path", { d: "m9 18 6-6-6-6", key: "mthhwq" }],
			]);
		},
		41585: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("triangle-alert", [
				[
					"path",
					{
						d: "m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3",
						key: "wmoenq",
					},
				],
				["path", { d: "M12 9v4", key: "juzpu7" }],
				["path", { d: "M12 17h.01", key: "p32p05" }],
			]);
		},
		41641: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("chevron-left", [
				["path", { d: "m15 18-6-6 6-6", key: "1wnfg3" }],
			]);
		},
		42869: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("user", [
				["path", { d: "M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2", key: "975kel" }],
				["circle", { cx: "12", cy: "7", r: "4", key: "17ys0d" }],
			]);
		},
		48368: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("file-text", [
				[
					"path",
					{
						d: "M6 22a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2z",
						key: "1oefj6",
					},
				],
				["path", { d: "M14 2v5a1 1 0 0 0 1 1h5", key: "wfsgrz" }],
				["path", { d: "M10 9H8", key: "b1mrlr" }],
				["path", { d: "M16 13H8", key: "t4e002" }],
				["path", { d: "M16 17H8", key: "z1uh3a" }],
			]);
		},
		57518: (e, r, t) => {
			t.d(r, { B8: () => E, UC: () => V, bL: () => z, l9: () => H });
			var a = t(12115),
				n = t(70379),
				i = t(68599),
				l = t(83478),
				c = t(83935),
				o = t(99354),
				d = t(1933),
				u = t(98979),
				s = t(89971),
				h = t(95155),
				y = "Tabs",
				[p, k] = (0, i.A)(y, [l.RG]),
				v = (0, l.RG)(),
				[f, m] = p(y),
				A = a.forwardRef((e, r) => {
					let {
							__scopeTabs: t,
							value: a,
							onValueChange: n,
							defaultValue: i,
							orientation: l = "horizontal",
							dir: c,
							activationMode: p = "automatic",
							...k
						} = e,
						v = (0, d.jH)(c),
						[m, A] = (0, u.i)({
							prop: a,
							onChange: n,
							defaultProp: i ?? "",
							caller: y,
						});
					return (0, h.jsx)(f, {
						scope: t,
						baseId: (0, s.B)(),
						value: m,
						onValueChange: A,
						orientation: l,
						dir: v,
						activationMode: p,
						children: (0, h.jsx)(o.sG.div, {
							dir: v,
							"data-orientation": l,
							...k,
							ref: r,
						}),
					});
				});
			A.displayName = y;
			var g = "TabsList",
				x = a.forwardRef((e, r) => {
					let { __scopeTabs: t, loop: a = !0, ...n } = e,
						i = m(g, t),
						c = v(t);
					return (0, h.jsx)(l.bL, {
						asChild: !0,
						...c,
						orientation: i.orientation,
						dir: i.dir,
						loop: a,
						children: (0, h.jsx)(o.sG.div, {
							role: "tablist",
							"aria-orientation": i.orientation,
							...n,
							ref: r,
						}),
					});
				});
			x.displayName = g;
			var b = "TabsTrigger",
				M = a.forwardRef((e, r) => {
					let { __scopeTabs: t, value: a, disabled: i = !1, ...c } = e,
						d = m(b, t),
						u = v(t),
						s = j(d.baseId, a),
						y = R(d.baseId, a),
						p = a === d.value;
					return (0, h.jsx)(l.q7, {
						asChild: !0,
						...u,
						focusable: !i,
						active: p,
						children: (0, h.jsx)(o.sG.button, {
							type: "button",
							role: "tab",
							"aria-selected": p,
							"aria-controls": y,
							"data-state": p ? "active" : "inactive",
							"data-disabled": i ? "" : void 0,
							disabled: i,
							id: s,
							...c,
							ref: r,
							onMouseDown: (0, n.mK)(e.onMouseDown, (e) => {
								i || 0 !== e.button || !1 !== e.ctrlKey
									? e.preventDefault()
									: d.onValueChange(a);
							}),
							onKeyDown: (0, n.mK)(e.onKeyDown, (e) => {
								[" ", "Enter"].includes(e.key) && d.onValueChange(a);
							}),
							onFocus: (0, n.mK)(e.onFocus, () => {
								let e = "manual" !== d.activationMode;
								p || i || !e || d.onValueChange(a);
							}),
						}),
					});
				});
			M.displayName = b;
			var w = "TabsContent",
				C = a.forwardRef((e, r) => {
					let { __scopeTabs: t, value: n, forceMount: i, children: l, ...d } = e,
						u = m(w, t),
						s = j(u.baseId, n),
						y = R(u.baseId, n),
						p = n === u.value,
						k = a.useRef(p);
					return (
						a.useEffect(() => {
							let e = requestAnimationFrame(() => (k.current = !1));
							return () => cancelAnimationFrame(e);
						}, []),
						(0, h.jsx)(c.C, {
							present: i || p,
							children: ({ present: t }) =>
								(0, h.jsx)(o.sG.div, {
									"data-state": p ? "active" : "inactive",
									"data-orientation": u.orientation,
									role: "tabpanel",
									"aria-labelledby": s,
									hidden: !t,
									id: y,
									tabIndex: 0,
									...d,
									ref: r,
									style: {
										...e.style,
										animationDuration: k.current ? "0s" : void 0,
									},
									children: t && l,
								}),
						})
					);
				});
			function j(e, r) {
				return `${e}-trigger-${r}`;
			}
			function R(e, r) {
				return `${e}-content-${r}`;
			}
			C.displayName = w;
			var z = A,
				E = x,
				H = M,
				V = C;
		},
		60285: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("ellipsis", [
				["circle", { cx: "12", cy: "12", r: "1", key: "41hilf" }],
				["circle", { cx: "19", cy: "12", r: "1", key: "1wjl8i" }],
				["circle", { cx: "5", cy: "12", r: "1", key: "1pcz8c" }],
			]);
		},
		60504: (e, r, t) => {
			t.d(r, { A: () => o });
			var a = t(12115),
				n = t(90901),
				i = t(44855),
				l = t(12180);
			let c = l.r
					? (e) => {
							e();
					  }
					: a.startTransition,
				o = (0, n.Ht)(i.Ay, () => (e, r, t = {}) => {
					let { mutate: i } = (0, n.iX)(),
						o = (0, a.useRef)(e),
						d = (0, a.useRef)(r),
						u = (0, a.useRef)(t),
						s = (0, a.useRef)(0),
						[h, y, p] = ((e) => {
							let [, r] = (0, a.useState)({}),
								t = (0, a.useRef)(!1),
								n = (0, a.useRef)(e),
								i = (0, a.useRef)({ data: !1, error: !1, isValidating: !1 }),
								c = (0, a.useCallback)((e) => {
									let a = !1,
										l = n.current;
									for (let r in e)
										Object.prototype.hasOwnProperty.call(e, r) &&
											l[r] !== e[r] &&
											((l[r] = e[r]), i.current[r] && (a = !0));
									a && !t.current && r({});
								}, []);
							return (
								(0, l.u)(
									() => (
										(t.current = !1),
										() => {
											t.current = !0;
										}
									)
								),
								[n, i.current, c]
							);
						})({ data: l.U, error: l.U, isMutating: !1 }),
						k = h.current,
						v = (0, a.useCallback)(async (e, r) => {
							let [t, a] = (0, l.s)(o.current);
							if (!d.current)
								throw Error("Can’t trigger the mutation: missing fetcher.");
							if (!t) throw Error("Can’t trigger the mutation: missing key.");
							let n = (0, l.m)(
									(0, l.m)({ populateCache: !1, throwOnError: !0 }, u.current),
									r
								),
								h = (0, l.o)();
							(s.current = h), p({ isMutating: !0 });
							try {
								let r = await i(
									t,
									d.current(a, { arg: e }),
									(0, l.m)(n, { throwOnError: !0 })
								);
								return (
									s.current <= h &&
										(c(() => p({ data: r, isMutating: !1, error: void 0 })),
										null == n.onSuccess || n.onSuccess.call(n, r, t, n)),
									r
								);
							} catch (e) {
								if (
									s.current <= h &&
									(c(() => p({ error: e, isMutating: !1 })),
									null == n.onError || n.onError.call(n, e, t, n),
									n.throwOnError)
								)
									throw e;
							}
						}, []),
						f = (0, a.useCallback)(() => {
							(s.current = (0, l.o)()), p({ data: l.U, error: l.U, isMutating: !1 });
						}, []);
					return (
						(0, l.u)(() => {
							(o.current = e), (d.current = r), (u.current = t);
						}),
						{
							trigger: v,
							reset: f,
							get data() {
								return (y.data = !0), k.data;
							},
							get error() {
								return (y.error = !0), k.error;
							},
							get isMutating() {
								return (y.isMutating = !0), k.isMutating;
							},
						}
					);
				});
		},
		61878: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("search", [
				["path", { d: "m21 21-4.34-4.34", key: "14j7rj" }],
				["circle", { cx: "11", cy: "11", r: "8", key: "4ej97u" }],
			]);
		},
		81262: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("printer", [
				[
					"path",
					{
						d: "M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2",
						key: "143wyd",
					},
				],
				["path", { d: "M6 9V3a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v6", key: "1itne7" }],
				["rect", { x: "6", y: "14", width: "12", height: "8", rx: "1", key: "1ue0tg" }],
			]);
		},
		84980: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("clock", [
				["path", { d: "M12 6v6l4 2", key: "mmk7yg" }],
				["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
			]);
		},
		89123: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("chart-column", [
				["path", { d: "M3 3v16a2 2 0 0 0 2 2h16", key: "c24i48" }],
				["path", { d: "M18 17V9", key: "2bz60n" }],
				["path", { d: "M13 17V5", key: "1frdt8" }],
				["path", { d: "M8 17v-3", key: "17ska0" }],
			]);
		},
		89803: (e, r, t) => {
			t.d(r, { b: () => u });
			var a = t(12115);
			t(47650);
			var n = t(42442),
				i = t(95155),
				l = [
					"a",
					"button",
					"div",
					"form",
					"h2",
					"h3",
					"img",
					"input",
					"label",
					"li",
					"nav",
					"ol",
					"p",
					"select",
					"span",
					"svg",
					"ul",
				].reduce((e, r) => {
					let t = (0, n.TL)(`Primitive.${r}`),
						l = a.forwardRef((e, a) => {
							let { asChild: n, ...l } = e;
							return (
								"u" > typeof window && (window[Symbol.for("radix-ui")] = !0),
								(0, i.jsx)(n ? t : r, { ...l, ref: a })
							);
						});
					return (l.displayName = `Primitive.${r}`), { ...e, [r]: l };
				}, {}),
				c = "horizontal",
				o = ["horizontal", "vertical"],
				d = a.forwardRef((e, r) => {
					var t;
					let { decorative: a, orientation: n = c, ...d } = e,
						u = ((t = n), o.includes(t)) ? n : c;
					return (0, i.jsx)(l.div, {
						"data-orientation": u,
						...(a
							? { role: "none" }
							: {
									"aria-orientation": "vertical" === u ? u : void 0,
									role: "separator",
							  }),
						...d,
						ref: r,
					});
				});
			d.displayName = "Separator";
			var u = d;
		},
		92622: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("funnel", [
				[
					"path",
					{
						d: "M10 20a1 1 0 0 0 .553.895l2 1A1 1 0 0 0 14 21v-7a2 2 0 0 1 .517-1.341L21.74 4.67A1 1 0 0 0 21 3H3a1 1 0 0 0-.742 1.67l7.225 7.989A2 2 0 0 1 10 14z",
						key: "sc7q7i",
					},
				],
			]);
		},
		93053: (e, r, t) => {
			t.d(r, { A: () => a });
			let a = (0, t(90425).A)("clipboard-check", [
				[
					"rect",
					{ width: "8", height: "4", x: "8", y: "2", rx: "1", ry: "1", key: "tgr4d6" },
				],
				[
					"path",
					{
						d: "M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2",
						key: "116196",
					},
				],
				["path", { d: "m9 14 2 2 4-4", key: "df797q" }],
			]);
		},
	},
]);

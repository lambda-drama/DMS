...
function save_submitted_doc(frm, field_map, callback) {
	...
	const set_next = (index) => {
		...
		frappe.call({
			...
			error: (err) => {
				frappe.msgprint(
					__("Failed to save field {0}: {1}", [fieldname, err.message || ""])
				);
			},
		});
	};
...
frappe.ui.form.on("DMS Job Card", {
	...
	onload_post_render(frm) {
		if (frm.is_new() && frm.doc.qc_checklist_template && !(frm.doc.qc_results || []).length) {
	...
	before_submit(frm) {
		...
		const missing = required.filter((r) => !frm.doc[r.field]).map((r) => r.label);

		if (missing.length) {
			frappe.throw(
				__(
					"Please fill in the following required fields before submitting:<br><b>{0}</b>",
					[missing.join(", ")]
				)
			);
		}
			...
			frappe.prompt(
				...
				(values) => {
					save_submitted_doc(
						...
						() => {
							frappe.show_alert(
								__("Customer approved. Submit button is now available."),
								5
							);
							control_submit_button(frm);
							frm.refresh();
	...
	frappe.db
		...
		.then((rows) => {
			...
			if (!can_create) {
				frappe.msgprint(
					__(
						"No Vehicle Delivery Note linked. You do not have permission to create one."
					)
				);
				return;
			}
...
function refresh_part_stock_available(cdt, cdn) {
	...
	frappe.call({
		method: "dms.dealer_management_system.doctype.dms_job_card.dms_job_card.get_job_card_part_stock_available",
		args: {
...
function refresh_part_unit_price(frm, cdt, cdn) {
	...
	frappe.call({
		method: "dms.dealer_management_system.doctype.dms_job_card.dms_job_card.get_job_card_part_unit_price",
		args: {
			spare_part,
		},
		callback: (r) => {
			...
			if (
				current_row.item_code === spare_part &&
				r.message !== undefined &&
				r.message !== null
			) {
...
function update_job_card_parts_total(frm) {
	...
	frappe.model
		.set_value(frm.doctype, frm.doc.name, "total_parts_cost", flt(total_parts, 2))
		.then(() => {
			const total_amount =
				flt(frm.doc.total_labor_cost || 0) + flt(frm.doc.total_parts_cost || 0);
			frappe.model
				.set_value(frm.doctype, frm.doc.name, "total_amount", flt(total_amount, 2))
				.then(() => {
					update_job_card_net_amount(frm);
				});
		});
...
function update_job_card_net_amount(frm) {
	...
	if (warranty_type === "All Invoice") {
		...
		frappe.show_alert(__("Set a labour and/or parts discount (total at least 1)."), 5);
	}
...
frappe.ui.form.on("Job Card QC Result", {
	measurement_value(frm, cdt, cdn) {
		...
		if (!pass) {
			...
			frappe.msgprint(
				__("Measurement {0} is outside the allowed range.", [row.measurement_value])
			);
		}
	},
			...
			() => {
				...
				if (critical_fails.length > 0) {
					frappe.msgprint({
						...
						message: __("{0} critical item(s) failed. Please fix before passing.", [
							critical_fails.length,
						]),
						indicator: "red",
					});
					return;
				...
				frappe.prompt(
					...
					(values) => {
						save_submitted_doc(
							...
							() => {
								frappe.show_alert(
									__("Road test failed. Sent back for rework."),
									3
								);
							}
						);
					...
					frappe.call({
						...
						callback: (r) => {
							...
							const msg = r.message?.material_issue
								? __("QC Passed. Material Issue {0} created.", [
										r.message.material_issue,
								  ])
		...
		frm.add_custom_button(
			...
			() => {
				save_submitted_doc(frm, { status: "Repair Completed" }, () => {
					frappe.show_alert(
						__("Rework completed. Ready for road test / QC re-check."),
						3
					);
				});
			},
...
function pause_repair(frm) {
	frappe.prompt(
		...
		(values) => {
			const now = frappe.datetime.now_datetime();
			const new_status =
				values.pause_reason === "Waiting Parts"
					? "Waiting Parts"
					: "Waiting Customer Approval";
			...
			if (!check.allowed) {
				frappe.msgprint({
					...
					message:
						check.reason ||
						__(
							"Request the parts or transfer the materials before completing the repair."
						),
					indicator: "orange",
				});
				return;
...
function confirm_stop_repair(frm) {
	frappe.confirm(__("Mark repair as completed? This will record all time logs."), () => {
		...
		const total_actual = (frm.doc.time_logs || []).reduce(
...
function populate_road_test_from_template(frm, opts = {}) {
	...
	return frappe.db.get_doc("Road Test Template", frm.doc.road_test_template).then((template) => {

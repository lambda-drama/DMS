...
class Technician(Document):
	...
	def update_performance_metrics(self):
		...
		job_cards = frappe.get_all(
			"DMS Job Card",
			filters={"technician": self.name, "docstatus": 1, "status": "Completed"},
		...
		if job_cards:
			...
			repeat_jobs = frappe.db.count(
				"DMS Job Card", {"technician": self.name, "is_repeat_repair": 1, "docstatus": 1}
			)
			if self.total_jobs_completed > 0:
				self.first_time_fix_rate = (
					(self.total_jobs_completed - repeat_jobs) / self.total_jobs_completed
				) * 100

			# Calculate average customer satisfaction
			satisfactions = [
				jc.get("customer_satisfaction", 0) for jc in job_cards if jc.get("customer_satisfaction")
			]
			if satisfactions:
	...
	def get_todays_jobs(self):
		...
		return frappe.get_all(
			"DMS Job Card",
			filters={"technician": self.name, "date": today_date, "docstatus": ["!=", 2]},
	...
	def is_available_now(self):
		...
		return (
			self.status == "Active"
			and self.attendance_today == "Present"
			and self.current_job_card is None
			and self.get_current_workload() < 3  # Max 3 concurrent jobs
		)

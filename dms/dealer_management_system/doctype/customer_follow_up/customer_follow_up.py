...
class CustomerFollowUp(Document):
	def on_update(self):
		...
		if self.repeat_repair_risk:
			frappe.db.set_value("Job Card", self.job_card, "is_repeat_repair", 1)
		
		if self.new_issue_reported and self.new_job_card_created:
			frappe.db.set_value("Job Card", self.job_card, "repeat_repair_reference", self.new_job_card_created)

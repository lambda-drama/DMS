...
frappe.ui.form.on("Spare Part", {
	refresh: function (frm) {
		...
		if (frm.doc.discontinued) {
			frm.dashboard.set_headline_alert(
				__(
					"This part has been DISCONTINUED. Superseded part: " +
						(frm.doc.superseded_by || "None")
				)
			);
		}
	},
	...
	is_hazardous_material: function (frm) {
		if (frm.doc.is_hazardous_material && !frm.doc.hazardous_class) {
			frappe.msgprint(
				__("Please enter the hazardous class for shipping/storage requirements.")
			);
		}
	},
});
